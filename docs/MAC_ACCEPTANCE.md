...
| Clean extracted launch | PASS | the executable extracted from the final ZIP started with isolated app data, reported version 1.4.1, served an empty library on loopback, and exposed no remote frontend dependency |
| Stable launch / stale-tab recovery | PASS | an immediate stop/replace/reopen reproduced the former restart race; 1.4.1 waited for and reclaimed `127.0.0.1:8765`. Static HTML/JS/CSS now use `no-store` plus versioned asset URLs |
...
| Bilingual recognition | PASS | the final extracted 1.4.1 package securely fetched and validated the linked upstream `json.zip`, cached 20,851 archive identifiers, and left no temporary download behind. Manga pages are never fetched or uploaded |
